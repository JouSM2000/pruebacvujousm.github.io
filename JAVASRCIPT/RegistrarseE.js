...
let requisitosEmail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
...
form.addEventListener('submit', e => {
  ...
  if(!requisitosEmail.test(email.value)){
  ...
  if(requisitosEmail.test(email.value)&&requisitosContraseña.test(contraseña.value)&&verificacion===true){
    entrada=true
  }else{
    entrada=false
  }
